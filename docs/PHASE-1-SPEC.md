...
> **Document ID** GYMAK-P1-SPEC-001 · **Version** 1.3 · **Date** 2 August 2026
...
> **Progress** T-01 to T-09c complete, backend done, T-10 next
>
...
>
> **Supersedes v1.0, v1.1 and v1.2 — do not use.** Any copy dated v1.0 still describes the
> 6-digit, user-id-salted reset code that P1-ADR-07 rejects. v1.1 and v1.2 predate the T-06
> through T-09c work recorded in the v1.3 amendments below. Delete any copy that is not this
> one. If two versions of this document are reachable by a coding agent, the
> single-source-of-truth rule is already broken.
...
| A-16 | §13.2 item 8                | Hosting criterion rewritten. It was never "offers a superuser migration path" — that premise was wrong (see A.5 item 11, closed in T-09b). The real test: the migration role can hold `CREATE` on the database, `citext` is not blocked by the provider's own extension allowlist, and the provider allows creating two separate roles. |
| A-17 | §4.7, §A.5 items 1, 6       | `gymak_migrator` / `gymak_app` role split shipped (T-09b). `gymak_app` no longer owns any table, so it can no longer drop its own RLS policies, disable `FORCE`, or re-grant itself privileges on `audit_log`. Closes A.5 items 1 and 6. |
| A-18 | §4.5, §A.5 item 14          | `chk_credential_present` replaced by a `DEFERRABLE INITIALLY DEFERRED` constraint trigger, `trg_users_credential_present` (migration `ae026cea6d8d`, T-09b), enforcing the real three-way invariant — password, OR verified email, OR a linked identity — at commit. Closes A.5 item 14; only became a real barrier once `gymak_app` stopped owning `users` (item 1, A-17). |
| A-19 | §5.10, §12 T-09             | `DELETE /account` shipped: password required when one exists, sets `deleted_at`, bumps `token_version`, revokes every refresh family, audits `account.deletion_requested` (T-09c-2). |
| A-20 | §6.4, §A.5 item 8           | Login rate limiting gained exponential backoff (T-09c-2): a violation while a lock is still active doubles the delay from a 30-second base, capped at one hour. `Retry-After` returns `max(window remainder, penalty)`, so it never promises earlier than the base window and the escalation only surfaces once the penalty outgrows the remainder. Closes A.5 item 8. |
| A-21 | §5.4, §7.3, §A.5 item 16    | Firebase misconfiguration — uninitialised SDK, certificate-fetch failure, revocation check unreachable — now returns `503 UPSTREAM_UNAVAILABLE` instead of `401 SOCIAL_TOKEN_INVALID` (T-09c-2). Closes A.5 item 16. |
| A-22 | §11.1, §11.3 item 3         | A generated cross-tenant test matrix (T-09c-3) enumerates every user-scoped route and asserts `404` when attempted as another user, closing P1-NFR-03's gate. |
| A-23 | §6.5, §11.1, §11.3 item 7   | A log-capture test (T-09c-3) asserts that no password, token, reset code, key material, birth date, height, or weight appears in any log line the full suite produces. |
| A-24 | §0.3, §11.1, §11.3 item 6   | `scripts/export_openapi.py` writes `openapi.json`; a test fails if the committed file drifts from the generated one (T-09c-3), closing P1-NFR-07's gate. |
| A-25 | §0.2, §A.5 item 17          | `mypy --strict` extended from `app/` to also cover `tests/` (T-09c-3): 33 type errors fixed without a single ignore. Closes A.5 item 17. |
...
| T-06 | Done   | Google sign-in only — decision 13.1.2 closed, Facebook deferred to Phase 2 (A-15). Dropped `chk_credential_present`; the real invariant later moved back into the database as a constraint trigger (T-09b, A-18). |
| T-07 | Done   | Three-call password reset on the ADR-07 primitive: console email backend, bilingual template, expiry/attempt-cap/reuse all enforced in SQL, not in Python after the fetch. |
| T-08 | Done   | `GET /auth/me`, `POST /profile`, `GET`/`PATCH /profile`, P1-SAF-001 enforced on both writes. Closed A.5 item 7 (`onboarding_completed` was hardcoded false). Follow-up fix: `height_cm`/`weight_kg` now serialise as JSON numbers, not strings. |
| T-09a | Done   | Hardening: real 1000-line password denylist from SecLists, Ed25519 keys actually parsed (not just shape-checked) at startup, `ruff format --check` added to the gate, `firebase.py` SDK init moved out of import time. Closed A.5 items 2, 3, 4, 15. |
| T-09b | Done   | `gymak_migrator` / `gymak_app` role split; `chk_credential_present` replaced by a `DEFERRABLE INITIALLY DEFERRED` constraint trigger. Closed A.5 items 1, 6, 11, 14 (A-17, A-18) — including the corrected finding that `CREATE EXTENSION citext` never needed a superuser (A-16). |
| T-09c-1 | Done | `bind_pre_auth_rls_user` helper unifies the three pre-auth RLS-binding call sites; RLS test fixtures decoupled from the policy under test. Closed A.5 items 9, 13; re-deferred item 12 with a reason. |
| T-09c-2 | Done | `DELETE /account`, login exponential backoff, Firebase misconfiguration now `503 UPSTREAM_UNAVAILABLE` instead of `401`, a structural test forcing the login dummy-hash branch. Closed A.5 items 8, 14(a), 16 (A-19, A-20, A-21). |
| T-09c-3 | Done | Generated cross-tenant test matrix, no-secret-logging log-capture test, `scripts/export_openapi.py` plus a committed `openapi.json` with a drift check, `mypy --strict` extended to `tests/`. Closed A.5 item 17 (A-22 through A-25). |
| T-10 | Next   | Mobile scaffold, theme, i18n. Backend (step 1 and step 2) is done; step 3 (mobile) begins here.            |
...
```text
You are working on Gymak, Phase 1 only. docs/PHASE-1-SPEC.md is your single source
of truth; if this document and your instinct disagree, the document wins. Ignore any
copy of this spec found anywhere else — nothing dated v1.0, v1.1 or v1.2 is current.
...
| 8. Hosting                                  | Local Docker Compose for Phase 1, staging after T-09. Railway or Fly.io with managed Postgres, per the SRS deployment baseline. **Answered concretely (A.5 items 1, 6, 11; A-16):** the criterion is not "offers a superuser migration path" — that premise was wrong. It is three things. **(a)** The migration role can hold `CREATE` on the *database* — `gymak_migrator` needs `CONNECT, CREATE` on the database and `CREATE, USAGE` on schema `public`; `citext` is a trusted extension on Postgres 13+, so database-level `CREATE` is enough to run `CREATE EXTENSION citext` without a superuser (confirmed empirically). **(b)** `citext` is not blocked by the provider's own extension allowlist — some managed hosts restrict installable extensions independently of the privileges above, and that allowlist is a separate thing to check before committing to a provider, not implied by (a). **(c)** The provider lets two separate, non-superuser roles be created — `gymak_migrator` and `gymak_app` (`CONNECT` on the database; `USAGE` only on schema `public`), with the exact grants in `backend/README.md`. A provider meeting all three qualifies; one that hands out only a single superuser-equivalent role, forbids creating additional roles, or blocks `citext` on its allowlist is disqualified. Do not spend time on production infrastructure until the auth flow works end to end on a device. |
...
Recorded here so it stops living only in chat history. Each item has an owning task. §11.3 item 11 requires every one of these to be closed or explicitly re-deferred, with a reason, before Phase 1 baselines — the table below shows that state as of this v1.3 baseline.
...
| 2 | **Password denylist — closed.** The vendored list had 313 hand-curated entries, not the ~1000 §6.2 asks for — "a fabricated list is worse than a short one, because it looks complete." Replaced with `app/core/data/common_passwords.txt`, exactly 1000 lines vendored byte-for-byte from SecLists (the URL named in §6.2 had 404'd upstream after a rename; retrieved 2026-08-01 from its replacement in the same directory/branch, `xato-net-10-million-passwords-1000.txt` — both the original request and the substitution are recorded in `app/core/security.py`'s comment above `_COMMON_PASSWORDS_PATH`, per the task's own "source and retrieval date recorded in a comment" instruction). | T-09a | Closed |
| 3 | **Real key parse at startup — closed.** `config.py` normalised and shape-checked the PEM but never called `load_pem_private_key`/`load_pem_public_key`, so a structurally valid, cryptographically broken key still failed only at first sign rather than at startup. Both loaders are now called from the `JWT_PRIVATE_KEY_PEM`/`JWT_PUBLIC_KEY_PEM` field validator, which also asserts the parsed key is actually Ed25519 (P1-ADR-02), not merely a well-formed key on some other curve. | T-09a | Closed |
| 4 | **`ruff format` — closed.** The gate ran `ruff check` only, so formatting drifted between tasks and was occasionally fixed as an unrelated side effect of a later diff. `ruff format --check` is now the second step of the documented gate (`backend/README.md`), after `ruff check` and before `mypy`/`pytest`, and the repository was reformatted once in the commit that added it. | T-09a | Closed |
...
| 7 | **`onboarding_completed` hardcoded false — closed.** Was correct only because nothing touched `profiles` yet, and would have gone silently wrong the moment T-08 shipped it. `auth_service._onboarding_completed_for` now looks the profile row up (`profile.onboarding_completed if profile is not None else False`) and every response that reports the flag — register, login, refresh, `/auth/me` — calls it, rather than hardcoding the value. `tests/integration/test_profile.py::test_onboarding_completed_flips_true_in_auth_me_login_and_refresh` proves it. | T-08 | Closed |
| 8 | **Login exponential backoff — closed.** §6.4 asks for backoff after 10 attempts per 15 minutes; `rate_limit.py` implemented fixed windows only, so tripping the limit only ever meant "wait out the rest of the 15-minute window," never an escalating penalty. `_InMemoryFixedWindow` now tracks a separate per-bucket violation streak (`bucket -> (streak, locked_until)`), consulted only for scopes named in `_ESCALATED_SCOPES` (`{"auth.login"}` today). Each violation while a lock is still active — including the one that first trips the base limit — doubles the delay from a 30-second base, capped at one hour. Both rejection branches of `consume` return `max(window remainder, penalty)`, so `Retry-After` never promises earlier than the base window itself allows; the escalation only becomes visible in the header once the doubling delay outgrows the remainder. Keyed identically to the base limit: `auth.py`'s two `enforce()` calls (`ip_key`, then `key_for_email`) are unchanged — `enforce()` decides whether to escalate from `scope` alone, so the call sites needed no edit, and "whichever trips first" (§6.4) still governs which bucket's streak advances. `test_login_backoff_escalates_retry_after_beyond_the_window_remainder` proves the first trip reports the 900s window remainder, not the smaller 30s penalty, and that the header only exceeds 900 once five further violations have compounded past it; `test_the_active_lock_retry_after_is_never_earlier_than_the_window` proves both the window-count and active-lock branches return the window remainder while it still dwarfs the penalty; `test_login_backoff_state_is_isolated_per_bucket` proves one bucket's escalation does not bleed into another's. | T-09c | Closed |
...
| 15 | **`firebase.py` initialises the SDK at import time — closed.** The same shape as the `database.py` defect T-04b fixed, minus the event-loop crash: importing `app.main` used to require a valid service-account JSON, so anything that merely imported the app (`scripts/export_openapi.py` included) failed without one. `init_firebase()` moved into `main.py`'s lifespan handler beside the privilege assertion (`firebase.py`'s own module docstring records this); building the OpenAPI schema, which never runs the lifespan, needs no credential as a result. | T-09a | Closed |
...
| 17 | **`mypy --strict` fails on `tests/` — closed.** 33 errors across seven test files by the time this was picked up (30 as originally scoped, plus 3 more from T-09c-2's work landing in between): 14 `no-untyped-def`, 9 `type-arg` (bare `-> dict` instead of `-> dict[str, Any]`), 7 `no-any-return` from `response.json()` leaking `Any`, 2 `operator` (`in` against `AppError.detail`, typed `str \| None`, without narrowing), and 1 `attr-defined` (`auth_service.verify_password` accessed as an attribute from the T-09c-2 dummy-hash test — an imported-not-defined name under `--no-implicit-reexport`). Every one fixed by correcting the type, not by adding an ignore: a new `tests/support.py` holds `JSONDict`/`json_body`, the one shared helper the `dict`/`no-any-return` pattern needed and that five files had each reimplemented independently; the `attr-defined` case was fixed by sourcing the real function from `app.core.security` (where it is actually defined) instead of via `auth_service`'s re-export, and patching through `monkeypatch.setattr`'s string-target form — no change to `app/services/auth_service.py`, which is outside this task's file list. `backend/README.md`'s documented gate now runs `mypy --strict .` against the whole tree. | T-09c-3 | Closed |
| 18 | **`_penalise` escalates on every rejected request while a lock is active, not once per window trip.** `app/core/rate_limit.py`'s exponential backoff (A.5 item 8) is deliberately punitive for the case it was built for — sustained password-guessing — but it does not distinguish a fresh violation from a request that arrives while an existing lock is still being served: `consume`'s `escalate` branch calls `_penalise` (and therefore advances the streak) both when the base window limit is first exceeded and, separately, whenever `_active_lock` finds a still-live lock, so a legitimate user who taps "log in" five times after one failed attempt reaches a 30 → 60 → 120 → 240 → 480 second delay, the same growth curve as five genuine, separately-timed attack attempts. Tightening, not a hole — the control still only ever penalises a caller for their own repeated requests, never someone else's — but user-hostile: a slow network causing a double-tap retry could plausibly compound this by accident, not just an attacker. Not fixed here, because §6.4 does not specify a de-duplication window and inventing one is a product/UX call, not a mechanical one. Revisit if it bites — e.g. by not advancing the streak for a request that arrives within some short window of the one that set the current lock, once a real double-tap-lockout report justifies picking that window's width. | — | Watch |

---

**End of GYMAK-P1-SPEC-001 v1.3** · Phase 1 only: authentication, session management, and one-time profile capture. Nothing in §1.2 gets built. Steps 1 and 2 (backend, T-01 … T-09c) are complete; §11.3 items 1–7, 9 and 11 are met. Item 10 is partially met — `backend/README.md` takes a fresh clone through the backend in under ten minutes, but there is no mobile half yet to verify, since T-10 has not shipped. Item 8 (the fourteen manual device checks) also waits on step 3. T-10 opens the mobile client.
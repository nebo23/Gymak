...
| 16 | **Firebase misconfiguration is reported to the client as a bad token.** `social_service.py` catches `Exception` broadly, so an uninitialised SDK — a deployment problem, not a client one — returns `401 SOCIAL_TOKEN_INVALID`, indistinguishable from a genuinely invalid token. §7.3 defines `503 UPSTREAM_UNAVAILABLE` for exactly this case. Narrow the catch so a configuration failure is not diagnosed as user error. | T-09c | Low |

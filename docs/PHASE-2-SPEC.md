...
> not a product — the home screen says so out loud: "your training plan arrives in the next phase."
...
>     Phase 1's §11.3 definition of done holds. In particular: the cross-tenant matrix is green and the RLS tests still fail when a policy is dropped.
>     `alembic upgrade head` runs clean on a fresh database, and `downgrade` back to base runs clean too.
>     The mobile app installs on a physical device and completes register → onboarding → home.
>     §11 of this document — the Phase 1 carry-over list — has been read, and each item is either fixed or consciously accepted. Two of them (the hardcoded LAN IP in `eas.json`, the untracked `mobile/android/`) will bite during Phase 2 if they are left alone.

## Decisions §13.1 are answered. The plan generator cannot be written while decision 1 is open.
...
Why A stored derived value is a value that can disagree with its inputs. Phase 1 already made this choice once — `age` is computed from `birth_date` and never stored (§5.8) — and the reasoning is identical here, with more arithmetic at stake. The formula also will change: Epley is a reasonable default and Brzycki is a defensible alternative, and switching a computed formula is a code change while switching a stored one is a backfill.
...
therefore reads through the parent (`EXISTS (SELECT 1 FROM workout_sessions s WHERE s.id = workout_sets.session_id AND s.user_id = current_app_user())`), and the cross-tenant test must
...
> Partial unique index: `CREATE UNIQUE INDEX ux_one_active_session ON workout_sessions (user_id) WHERE status = 'in_progress';` — P2-ADR-03, enforced by the database.
> 4.7 workout_sets
> Column Type Constraints Notes
> id uuid PK
...
client offers Resume / Discard and start new rather than a dead end. `404 NOT_FOUND` if the
...
`is_record` Computed against the user's history excluding this session's other sets, so a new personal best is announced once, not on every subsequent heavier set in the same workout. The celebration is the app's; the fact is the server's.
...
Arms & delts isolation(side_delts) · isolation(rear_delts) · isolation(biceps) · isolation(triceps) · isolation(abs)
...
The current exercise is always visible without scrolling, along with its target (`4 × 6–8`),
its rest time, and what the user did last time. Everything else may scroll.
Logging a set is at most two taps from resting state when the values are unchanged from the
...
first set.
Number entry is a `GNumberField`, not a raw keyboard. Steppers (± 2.5 kg, ± 1 rep) with a
tap-to-type fallback. `keyboardType="decimal-pad"`. The field never loses focus to a re-render.
The rest timer starts automatically when a set is logged, seeded from the exercise's
...
A logged set appears instantly (optimistic), marked as unsent until the server confirms it
(P2-ADR-08). A failed send shows a retry chip on that set, and never a modal.
Navigating away is guarded. The back gesture and the tab bar are blocked while a session is
...
Rotation and background do not lose state, because state lives in `src/workout/activeSession.ts`
and the server, never only in a component.
Finish shows a summary before it commits: duration, volume, sets, and any records set.
...
"Gymak is not medical advice. Stop if something hurts, and talk to a doctor before starting a new
programme." Not a modal, not a one-time dismissal. One quiet line (P2-SAF-004).
...
> Fresh account, complete onboarding → dashboard shows the empty state with "build my plan", no crash, no zeros pretending to be data.
> Generate a 4-day plan → four day cards, each with focus chips and an exercise count.
> Open day 1 → exercises in order, compounds first, targets shown, `last_performance` absent on the first ever run.
> Start the workout → the active screen opens, tabs are hidden, back is guarded.
> Log a set → it appears instantly, the rest timer starts, `session_totals` update.
> Background the app for 60 seconds mid-rest, return → the timer shows the correct remaining time, not a frozen one.
> Rotate the device mid-session → no set is lost, the current exercise is unchanged.
> Turn wifi off, log two sets, turn it back on → both sets show as unsent, then send on retry; Finish is blocked until they do.
> Log a heavier set than any before → the record is announced once, not on every subsequent set.
> Finish → the summary shows duration matching the actual training time, not the wall clock since Start.
> Reopen the app → the dashboard streak reads 1, this-week shows 1 of 4.
> Log body weight for today, then again with a different number → one entry, the second value, and Settings shows the new weight.
> Change weight in Settings → the Progress chart shows today's point at the new value.
> Switch to English, reload → every Phase 2 screen is LTR with no clipped labels and no leftover Arabic; switch back → RTL throughout, including the chart's axis labels.
> Set the device font size to 130% → the active-workout screen still fits, nothing clips, the number fields are still usable.
> Dark mode across every new screen → no invisible text, no white flash on navigation.
> TalkBack on → a full set can be logged, and the rest timer announces completion.
> A 16-year-old account (birth date set accordingly) → no weight-loss framing anywhere on the dashboard or in the plan; the weight chart still renders.
...
- Finish by listing: files touched, what changed, how to verify, and anything you
  ...
  ruff format --check, mypy --strict . and pytest.
- Commit and push when, and only when, that whole gate is green. One commit per
  task on branch claude/gymak-weight-tracker-review-76pvgl, its subject naming
  the task, e.g. "feat(workouts): T-18 session lifecycle". Never commit a red
  gate. Never amend, rebase, or force-push a commit that is already on origin.
```

> **The commit rule changed after T-17b, deliberately.** Through T-15 to T-17b the preamble
> ended in "Do not commit", so every diff was read before it landed. That caught real things —
> T-17's ceiling conflict among them. It was dropped because the review was happening in the
> report rather than in the diff anyway, and the manual step had become pure friction.
>
> What replaces it: **one commit per task**, so any single task can be reverted without
> untangling it from another, and **never on a red gate**, so `origin` always holds a state
> whose tests passed. If a task's report says something surprising, `git show` is still there —
> the review moved, it did not disappear.
...
Backend: `tzdata`. Added in T-18. `zoneinfo` is a standard-library API with no IANA data of its
own — on Windows and on slim/alpine base images, `zoneinfo.available_timezones()` returns empty
without it, which silently rejects every timezone the §7.1 validator is supposed to accept. Not
optional once §4.2's `timezone` column exists.
